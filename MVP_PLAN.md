...
### Umsetzungsreihenfolge (hochlevel)
...
### Migration Python → TypeScript (verbindlich)
- Module zu portieren: `docs_generator.py` → `src/generator/*`, `docs_validator.py` → `src/validator/*`, `docs_search.py` → `src/index/*`, `universal_scanner.py` → `src/core/*`.
- Nach erfolgreicher Funktionsparität: alle Python-Dateien und -Verweise löschen, Settings für Python entfernen, keine Prozess-Spawns zu Python.
